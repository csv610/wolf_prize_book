...
\begin{enumerate}
    \item \textbf{Lewy's example}\index{Lewy example} (1957): a linear PDE with smooth coefficients having no solutions, which revolutionized the understanding of solvability.
...
\subsection{CR Geometry\index{CR geometry} and Extension}
...
\subsection{The \texorpdfstring{$\overline{\partial}$}{d-bar} Problem and CR Geometry}
...
\subsection{The \texorpdfstring{$\overline{\partial}$}{d-bar}-Neumann Problem}
...
\subsection{The \texorpdfstring{$\overline{\partial}$}{d-bar}-Neumann Problem and Subelliptic Estimates}
...
\addcontentsline{toc}{section}{Exercises}\begin{enumerate}
    \item [B] Verify that the Lewy operator $L = \partial/\partial \overline{z} - i z \partial/\partial t$ is not elliptic by computing its symbol.
    \item [I] Show that if $f$ is real-analytic, the Lewy equation has a local solution, using the Cauchy--Kovalevskaya theorem.
    \item [I] Prove the Hopf boundary point lemma for the Laplace operator $\Delta$ using the Poisson integral formula.
    \item [B] Show that a harmonic function on a bounded domain that attains its maximum at an interior point must be constant.
    \item [B] Show that the Cauchy--Riemann equations $\partial u/\partial \overline{z} = 0$ are elliptic.
    \item [I] Compute $L\overline{L} - \overline{L}L$ for the Lewy operator and show it is non-zero.
    \item [I] Show that the operator $\partial/\partial t + i \partial/\partial x$ on $\mathbb{R}^2$ is locally solvable (it's conjugate to the $\overline{\partial}$ operator).
    \item [I] Show that $\overline{\partial}_b$ is the tangential part of $\overline{\partial}$ by restricting to a hypersurface $\rho = 0$.
    \item [I] Prove that a CR function on the sphere $S^{2n-1} \subset \mathbb{C}^n$ extends holomorphically to the unit ball.
    \item [A] Show that if $M$ is pseudoconcave, CR functions on $M$ may not extend holomorphically.
    \item [I] Derive the Hopf--Lewy lemma for $\Delta u \geq 0$ using the Kelvin transform.
    \item [B] Show that the Monge--Amp\`{e}re equation on $\mathbb{R}^2$ can be written as the determinant of the Hessian.
    \item [A] Show that the Lewy operator is of principal type but not of real principal type.
    \item [A] Compute the symbol of the Kohn Laplacian $\square_b = \overline{\partial}_b^* \overline{\partial}_b + \overline{\partial}_b \overline{\partial}_b^*$ on the Heisenberg group.
    \item [I] Show that the Nirenberg--Treves condition (P) is satisfied by every elliptic operator.
\end{enumerate}
...
1904 & Born Hans Lewy on October 20 in Breslau, Germany (now Wrocław, Poland). \\ \hline
1926 & Earns Ph.D. from the University of Göttingen under Richard Courant. \\ \hline
1928 & Co-authors the seminal paper introducing the Courant-Friedrichs-Lewy (CFL) condition, a fundamental stability criterion for numerical solutions of PDEs. \\ \hline
1933 & Flees Nazi Germany, moving first to Italy, then to France. \\ \hline
1935 & Emigrates to the United States, taking a position as a lecturer at Brown University. \\ \hline
1937 & Joins the faculty of the University of California, Berkeley, where he would spend the remainder of his career. \\ \hline
1957 & Publishes "An example of a smooth linear partial differential equation without solution," known as Lewy's example, demonstrating the existence of linear PDEs with smooth coefficients that have no local solutions. This was a major counterexample in the theory of PDEs. \\ \hline
1964 & Elected a member of the National Academy of Sciences. \\ \hline
1986 & Awarded the Wolf Prize in Mathematics (shared with Samuel Eilenberg) for his pioneering work on partial differential equations. \\ \hline
1988 & Dies on August 23 in Berkeley, California. \\ \hline
\end{tabularx}
...
\begin{enumerate}
    \item Lewy, H. (1957). An example of a smooth linear partial differential equation without solution. \textit{Annals of Mathematics}, 66(1), 155--158.
    \item Courant, R., Friedrichs, K. O., \& Lewy, H. (1928). Über die partiellen Differenzengleichungen der mathematischen Physik. \textit{Mathematische Annalen}, 100(1), 32--74.
    \item Lewy, H. (1938). On the reflection law of solutions of partial differential equations in two independent variables. \textit{Bulletin of the American Mathematical Society}, 44(1), 1--7.
\end{enumerate}

\subsection*{Biographies and Overviews}
\begin{enumerate}
    \item Morawetz, C. S. (1992). Hans Lewy (1904--1988). In \textit{Biographical Memoirs of the National Academy of Sciences}, Vol. 61, pp. 273--284. National Academies Press.
    \item Nirenberg, L. (1989). Hans Lewy. \textit{Bulletin of the American Mathematical Society}, 21(1), 153--157.
    \item Bers, L. (1989). Hans Lewy. \textit{Notices of the American Mathematical Society}, 36(1), 13--15.
\end{enumerate}

\subsection*{Textbooks and Expository Works}
\begin{enumerate}
    \item Hörmander, L. (1983). \textit{The Analysis of Linear Partial Differential Operators I: Distribution Theory and Fourier Analysis}. Springer-Verlag.
    \item Evans, L. C. (2010). \textit{Partial Differential Equations} (2nd ed.). American Mathematical Society.
    \item Treves, F. (1975). \textit{Basic Linear Partial Differential Equations}. Academic Press.
\end{enumerate}
