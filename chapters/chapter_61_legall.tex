...
\begin{enumerate}
    \item Show that the CRT encoded by a Brownian excursion is a random metric tree.
    \item Compute the Hausdorff dimension of the Brownian snake path.
    \item Show that the number of planar quadrangulations with $n$ faces grows as $c \cdot 12^n n^{-7/2}$.
    \item Prove that the Brownian map is a geodesic metric space.
    \item Compute the spectral dimension of the CRT.
    \item Show that super-Brownian motion satisfies the branching property.
    \item Prove that the Brownian map has Hausdorff dimension 4.
\end{enumerate}
