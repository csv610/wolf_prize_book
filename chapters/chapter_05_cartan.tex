...
\begin{enumerate}
    \item Show that $\mathbb{C}^n$ is a Stein manifold by verifying holomorphic convexity and separation of points.
    \item Let $\cO$ be the sheaf of holomorphic functions on $\mathbb{C}$. Show that the exponential sequence $0 \to \mathbb{Z} \to \cO \to \cO^\times \to 0$ is exact.
    \item Compute $H^1(\mathbb{C}^\times, \cO^\times)$ using the exponential sequence and the fact that $H^1(\mathbb{C}^\times, \cO) = 0$.
    \item Let $F: \cA \to \cB$ and $G: \cB \to \cC$ be left exact functors between abelian categories with enough injectives. If $F$ sends injectives to $G$-acyclic objects, derive the Grothendieck spectral sequence.
    \item Show that for a free abelian group $\mathbb{Z}^n$, $H^*(\mathbb{Z}^n, \mathbb{Z})$ is an exterior algebra on $n$ generators of degree 1.
    \item Compute the Serre spectral sequence for the Hopf fibration $S^1 \to S^3 \to S^2$ and show it collapses at $E_2$.
    \item Show that if $f: X \to Y$ is a covering map, then $H^q(X, \cF) \cong H^q(Y, f_*\cF)$ for any sheaf $\cF$ on $X$.
    \item Prove that the structure sheaf $\cO$ on $\mathbb{C}^n$ is a coherent sheaf of rings.
    \item Show that for a projective module $P$, the functor $\Hom(P, -)$ is exact.
    \item Let $G$ be a finite group. Show that $H^n(G, M) = 0$ for all $n \geq 1$ and all $\mathbb{Q}[G]$-modules $M$.
    \item Compute the Adem relation $Sq^1 Sq^2 + Sq^2 Sq^1 = 0$ in the Steenrod algebra $A^2$ by applying the relation to the class of degree 2 in $H^*(K(\mathbb{Z}_2, 1); \mathbb{Z}_2)$.
    \item Let $X$ be a compact Riemann surface of genus $g$. Show that $H^1(X, \cO) \cong \mathbb{C}^g$ using the Dolbeault isomorphism and Hodge theory.
\end{enumerate}
