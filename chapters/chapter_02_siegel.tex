...
\begin{enumerate}
    \item Prove Dirichlet's approximation theorem using the pigeonhole principle.
    \item Show that Liouville's theorem implies $\sum_{n=1}^\infty 10^{-n!}$ is transcendental.
    \item State and prove Siegel's lemma for $M=1$ equation in $N$ variables. What is the bound?
    \item Show that the curve $y^2 = x^3 - 2$ has genus 1 by computing the discriminant.
    \item For the quadratic form $Q(x,y) = x^2 + y^2$, compute the local densities $\alpha_2(Q,1)$ and $\alpha_\infty(Q,1)$. Verify the mass formula for $m=1$.
    \item Show that $\mathcal{H}_n$ is a domain of holomorphy.
    \item Prove that a Siegel modular form of weight $k$ on $\mathcal{H}_n$ can be expressed as a Fourier series $F(\Omega) = \sum_{T \geq 0} a_T e^{2\pi i \operatorname{Tr}(T\Omega)}$, where $T$ runs over half-integral positive semidefinite symmetric matrices.
    \item Show that if $f$ is an E-function and $\alpha$ is algebraic, then the derivative $f'$ is also an E-function.
    \item Let $C: y^2 = x^3 - 2x$. Compute its genus and find three integer points on $C$. Show that $C$ is an elliptic curve.
    \item For the Bessel function $J_0(z) = \sum_{n=0}^\infty \frac{(-1)^n}{(n!)^2} \left(\frac{z}{2}\right)^{2n}$, prove that $y = J_0(z)$ satisfies $z^2 y'' + z y' + z^2 y = 0$. Show that $J_0(z)$ is an E-function.
    \item Let $Q(x,y,z) = x^2 + y^2 + z^2$. Using the mass formula, show that the number of integer solutions to $x^2 + y^2 + z^2 = m$ is approximately $4\pi m$ for large $m$.
    \item Prove that the symplectic group $Sp(2n,\mathbb{R})$ acts transitively on $\mathcal{H}_n$ via the action $\Omega \mapsto (A\Omega + B)(C\Omega + D)^{-1}$.
\end{enumerate}
