...
David Bryant Mumford was born in 1937 in Sussex, England. He studied mathematics at Harvard University, where he received his Ph.D.\ in 1961 under the supervision of Oscar Zariski. He spent most of his career at Harvard, becoming a professor in 1967. He was awarded the Fields Medal in 1974 for his work on algebraic surfaces and moduli spaces. His later research shifted to computer vision and pattern theory at Brown University. He received the Wolf Prize in Mathematics in 2008.
...
\begin{itemize}
    \item \textbf{Geometric Invariant Theory\index{geometric invariant theory} (GIT):} A systematic framework for constructing quotients of algebraic varieties by group actions, which became the foundation of modern moduli theory.
...
\begin{enumerate}
    \item [I] Show that the moduli space $M_{1,1}$ of elliptic curves is isomorphic to $\mathbb{A}^1$ via the $j$-invariant, and that its Deligne--Mumford compactification is $\mathbb{P}^1$.
    \item [I] Let $\mathrm{PGL}(n)$ act on $\mathbb{P}^n$ by linear transformations. Determine the stable and semi-stable points using the Hilbert--Mumford criterion.
    \item [I] Prove that a stable curve of genus $g \geq 2$ has finite automorphism group.
    \item [I] Show that the Mumford--Shah functional is lower semi-continuous with respect to appropriate topologies on $u$ and $K$.
    \item [I] Let $A$ be an abelian variety and $L$ an ample line bundle. Show that the theta group of $(A, L)$ is a finite Heisenberg group.
    \item [I] Classify the possible configurations of rational components in a stable curve of genus $2$.
    \item [B] Show that the Hilbert--Mumford criterion implies that a point $x$ in $\mathbb{P}^n$ is $\SL_{n+1}$-stable if and only if it has no non-zero coordinates equal to zero.
\end{enumerate}
