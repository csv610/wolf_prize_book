...
Upon completing his PhD, Milnor joined the Princeton faculty. He became a full professor in 1960. During his early years at Princeton, he made a series of discoveries that transformed differential topology. In 1956, he stunned the mathematical world by proving the existence of exotic spheres --- smooth manifolds homeomorphic but not diffeomorphic to the standard 7-sphere. This work earned him the Fields Medal at the 1962 International Congress of Mathematicians in Stockholm.
...
\subsection{The Classification of \(S^3\)-Bundles over \(S^4\)}
...
\subsection{The Invariant \(\lambda(M)\)}
...
\subsection{The \(h\)-Cobordism Theorem and Exotic Spheres}

The \(h\)-cobordism theorem, proved by Smale in the early 1960s, is intimately connected with Milnor's work on exotic spheres. An \(h\)-cobordism between two manifolds \(M\) and \(N\) is a manifold \(W\) whose boundary is \(M \sqcup N\) such that both \(M\) and \(N\) are deformation retracts of \(W\). Smale proved that for simply connected manifolds of dimension \(n \geq 5\), an \(h\)-cobordism implies a diffeomorphism.
...
The Milnor fibration is one of the central objects in singularity theory. It describes the topology of a complex hypersurface near an isolated singularity.
...
The intersection form on the middle homology \(H_n(F_f; \Z)\) is non-degenerate and has signature determined by the signature of the singularity. The \textbf{vanishing cycles} --- homology classes in \(H_n(F_f; \Z)\) that are annihilated by the inclusion of the fiber into the total space --- play a crucial role in the Picard--Lefschetz theory.
...
\begin{enumerate}
    \item Compute the Milnor number \(\mu\) for the singularity \(f(x,y) = x^p + y^q\) in \(\C^2\). Show that \(\mu = (p-1)(q-1)\).
    \item For the \(A_k\) singularity \(f(z_0,\dots,z_n) = z_0^{k+1} + \sum_{i=1}^n z_i^2\), compute the Milnor number and describe the Milnor fiber.
    \item Let \(M^7\) be an \(S^3\)-bundle over \(S^4\). Show that if the Euler class \(h\) and the Pontryagin number satisfy \(h + j = 1\), then \(M^7\) is homeomorphic to \(S^7\). (Use the Hurewicz theorem and the Whitehead theorem.)
    \item Prove that the free group on two generators \(F_2\) has exponential growth, and show that it is not virtually nilpotent.
    \item Let \(G\) be a finitely generated nilpotent group of nilpotency class \(c\). Show that \(\gamma_G(n) \leq C n^c\) for some constant \(C\).
    \item Consider the trefoil knot in \(\R^3\). Compute its total curvature by approximating with a polygonal knot. Verify that the total curvature exceeds \(4\pi\).
    \item Show that the standard 7-sphere \(S^7\) bounds an 8-manifold with signature 0, while the exotic sphere \(M^7_{1,0}\) bounds an 8-manifold with signature 1 (mod 7). Conclude that \(\lambda(M^7_{1,0}) \neq 0\).
    \item For a flat \(\SL(2,\R)\)-bundle over a surface \(\Sigma_g\) with representation \(\rho: \pi_1(\Sigma_g) \to \SL(2,\R)\), prove that the Euler number satisfies \(|e(\rho)| \leq 2g - 2\). (Hint: pull back to the universal cover and consider the associated disk bundle.)
    \item Let \(f(x,y) = x^3 + y^4\). Describe the Milnor fiber of this singularity and compute its homology.
    \item Show that \(\Z \oplus \Z\) has polynomial growth of degree 2, while the Heisenberg group (the group of \(3 \times 3\) upper triangular integer matrices with ones on the diagonal) has polynomial growth of degree 4.
    \item Prove the Fenchel theorem: for any simple closed curve in \(\R^3\), the total curvature is at least \(2\pi\). (Hint: the tangent indicatrix is a closed curve on \(S^2\) whose length equals the total curvature; a closed curve on \(S^2\) has length at least \(2\pi\).)
    \item Compute \(K_2(\C)\) using Matsumoto's theorem. Show that \(K_2(\C)\) is divisible (every element is an \(n\)-th power for any \(n\)).
    \item For the \(D_4\) singularity \(f(x,y,z) = x^3 + x y^2 + z^2\), compute the Milnor number and the intersection form on the middle homology of the Milnor fiber.
    \item Show that the fundamental group of a closed oriented surface \(\Sigma_g\) with \(g \geq 2\) has exponential growth.
    \item Let \(\rho: \pi_1(\Sigma_g) \to \SL(2,\R)\) be a discrete and faithful representation corresponding to a hyperbolic structure on \(\Sigma_g\). Show that the Euler number of the associated flat bundle equals \(\pm (2g-2)\).
    \item Let \(f(z_0,z_1) = z_0^3 + z_1^5\). Compute the Milnor number of this singularity. Draw the vanishing cycles on the Milnor fiber and describe the monodromy.
    \item Prove that the group of orientation-preserving diffeomorphisms of the standard 7-sphere does not act transitively on the set of all differentiable structures on \(S^7\).
    \item Let \(G\) be a finitely generated group. Show that if \(G\) has subexponential growth, then every finitely generated subgroup also has subexponential growth.
    \item Consider the quadratic map \(f_c(z) = z^2 + c\). Show that if \(|c| > 2\), then \(c\) lies outside the Mandelbrot set (i.e., \(f_c^n(0) \to \infty\)).
\end{enumerate}
