...
\begin{itemize}
    \item \textbf{The Weil Conjectures (1949):} A set of conjectures describing the zeta function\index{zeta function, Weil} of an algebraic variety over a finite field, including an analogue of the Riemann hypothesis. These conjectures drove the development of modern algebraic geometry for 25 years.
...
\begin{proof}[Sketch]
\begin{enumerate}
    \item Grothendieck (1964): Constructed \'{e}tale cohomology $H_{\text{\'{e}t}}^i(V_{\overline{\F}_q}, \mathbb{Q}_l)$ and used the Lefschetz fixed point formula to express $|V(\F_{q^n})|$ as an alternating sum of traces of Frobenius, giving rationality.
...
For further exploration, see Chapter~46 (Deligne) on algebraic geometry and Weil conjectures, Chapter~30 (Langlands) on number theory and automorphic forms, and Chapter~31 (Wiles) on modular forms and Fermat\'s Last Theorem.

\section*{Exercises for the Reader}
\addcontentsline{toc}{section}{Exercises}

\begin{enumerate}
    \item [B] Let $C: y^2 = x^3 - x$ over $\F_3$. Compute the number of $\F_3$-rational points and verify the Hasse--Weil bound.
    \item [I] Show that for an elliptic curve $E$ over $\F_q$, the zeta function satisfies $Z_E(t) = \frac{1 - a_1 t + q t^2}{(1-t)(1-qt)}$ where $a_1 = q + 1 - N_1$.
    \item [A] Prove that if $E$ is an elliptic curve over $\F_q$, then $|a_1| \leq 2\sqrt{q}$ (the Hasse bound) by computing the positivity of the intersection pairing $\Delta \cdot \Gamma_F$ on $E \times E$.
    \item [I] Let $\Q$ be the rational numbers. Show that the idele group $\mathbb{A}_\Q^\times$ is $\mathbb{R}_{>0} \times \widehat{\mathbb{Z}}^\times \times \prod_p \mathbb{Z}_p^\times$ modulo discrete embeddings.
    \item [B] Show that for a smooth projective curve $C$ of genus $g$ over $\mathbb{C}$, $b_0 = b_2 = 1$ and $b_1 = 2g$.
    \item [I] State the Mordell--Weil theorem for an elliptic curve $E/\Q$ and show that $E(\Q)/2E(\Q)$ is finite using elementary descent.
    \item [B] Let $\F_q$ be a finite field. Define the Frobenius morphism $\Frob: \mathbb{P}^1_{\F_q} \to \mathbb{P}^1_{\F_q}$. Compute the number of fixed points of $\Frob^n$ directly.
    \item [I] Prove the functional equation for the zeta function of $\mathbb{P}^1$ over $\F_q$.
    \item [I] Show that the Weil representation for $SL(2,\mathbb{R})$ acting on $L^2(\mathbb{R})$ is given by:
    \[
    ...
    \item [I] Let $C$ be a curve of genus $2$ over $\F_2$ with $N_1 = 5$. Compute $N_2$ and $N_3$ using the Weil conjectures.
    \item [A] Show that the group $E(\mathbb{Q})_{\text{tors}}$ of torsion points on an elliptic curve $E/\mathbb{Q}$ is finite (Mazur's theorem classifies the possibilities, but just prove finiteness using basic height theory).
    \item [I] Compute the zeta function of $\mathbb{P}^n$ over $\F_q$ and verify the Weil conjectures for it explicitly.
\end{enumerate}
