...
\begin{enumerate}
    \item Compute the Kazhdan--Lusztig polynomials for the symmetric group $S_3$.
    \item Show that the Hecke algebra $H_q(S_n)$ specializes to $\C[S_n]$ when $q = 1$.
    \item Compute the left cells of $S_3$ and describe the corresponding cell representations.
    \item Show that the canonical basis for $U_q(\mathfrak{sl}_2)$ consists of elements $E^{(n)} = E^n/[n]_q!$.
    \item Prove that a character sheaf on $GL(2)$ gives the characters of $GL(2, \F_q)$.
    \item Show that the Kazhdan--Lusztig polynomials for the dihedral group $D_4$ are all $1$.
    \item Compute the irreducible characters of $\SL(2, \F_q)$ using Deligne--Lusztig theory.
\end{enumerate}
