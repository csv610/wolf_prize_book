...
\begin{enumerate}
    \item Show that the Loewner equation generates a simple curve for $\kappa \leq 4$.
    \item Compute the fractal dimension of SLE$_4$ and SLE$_6$.
    \item Show that LERW on a finite graph is the uniform spanning tree path between two points.
    \item Prove that intersection exponents satisfy a certain functional equation (cascade relation).
    \item Show that SLE$_{8/3}$ satisfies the restriction property.
    \item Compute the probability that SLE$_6$ passes to the left of a given point.
    \item Show that the Gaussian free field is related to SLE$_4$.
\end{enumerate}
