...
\begin{enumerate}
    \item Use the combinatorial Nullstellensatz to prove that any graph of maximum degree $\Delta$ is $(\Delta+1)$-colorable.
    \item Prove the Alon--Boppana bound for $d$-regular trees.
    \item Show that the $(n,d,\lambda)$-graph property implies the expander mixing lemma.
    \item Apply the probabilistic method to show existence of a $k$-coloring of a graph with high girth.
    \item Prove the Erdos--Ginzburg--Ziv theorem for $n$ prime.
    \item Show that the Alon--Tarsi theorem implies that even cycles are 2-choosable.
    \item Construct a Ramanujan graph using Cayley graphs of $\PSL(2,q)$.
\end{enumerate}
