...
\begin{itemize}
    \item \textbf{Eilenberg--Mac Lane Spaces:} For any group $G$ and integer $n \geq 1$, the space $K(G,n)$ whose only non-vanishing homotopy group is $\pi_n(K(G,n)) \cong G$. These spaces represent cohomology: $H^n(X; G) \cong [X, K(G,n)]$.
...
\subsection{Eilenberg--Mac Lane Spaces $K(G,n)$}
...
\begin{enumerate}
    \item Show that $H_n(X \times Y) \cong H_n(X) \otimes H_n(Y)$ when both $X$ and $Y$ are $0$-dimensional CW complexes.
    \item Let $G$ be an abelian group. Prove that $K(G,1)$ exists as the classifying space $BG$, and compute $\pi_1(BG)$.
    \item Verify that singular homology satisfies the Eilenberg--Steenrod axioms. Which axiom is the most difficult to verify?
    \item For a finite group $G$, show that $H_n(K(G,1); \Z)$ is torsion for $n \geq 1$.
    \item Use the Eilenberg--Zilber theorem to prove the K\"unneth formula for spaces with finitely generated homology.
    \item Compute $\Tor^{\Z[x]}(\Z,\Z)$ where $|x| = 2$ and $H^*(B) = \Z[x]$. Show that it recovers $H^*(S^1)$.
    \item Prove the Yoneda lemma: $\Nat(\Hom(A,-), F) \cong F(A)$ for a small category.
    \item Let $F: \mathcal{C} \to \mathcal{D}$ be a functor. Show that $F$ is an equivalence of categories iff it is fully faithful and essentially surjective.
    \item Compute $\Ext_{\Z}^n(\Z/m\Z, \Z)$ for all $n \geq 0$.
    \item Show that the Eilenberg swindle implies that every finitely generated projective module over a local ring is free.
    \item Prove that if $A_* \to B_* \to C_*$ is a short exact sequence of chain complexes, there is a long exact sequence of homology groups.
    \item Show that $K(\Z,2) \cong \mathbb{C}P^\infty$ and use this to prove that $H^2(X;\Z) \cong [X, \mathbb{C}P^\infty]$ classifies complex line bundles.
    \item For a triple $(X,A,B)$ with $B \subset A \subset X$, derive the long exact sequence of the triple using the Eilenberg--Steenrod axioms.
    \item Prove the five lemma: if the rows of a commutative ladder diagram of abelian groups are exact and four of the five vertical arrows are isomorphisms, then the fifth is also an isomorphism.
    \item Show that the Alexander--Whitney map and the shuffle map are chain homotopy inverses for $X = Y = \Delta^1$.
\end{enumerate}
