...
\begin{enumerate}
    ...
    \item [B] Implement Shamir's $(3,5)$-threshold secret sharing over a finite field.
    \item [I] Show that breaking RSA by computing $\varphi(n)$ is as hard as factoring $n$.
    \item [I] Compute the probability of a differential characteristic for a simple 2-round Feistel cipher.
    \item [A] Show that the Feige--Fiat--Shamir protocol is a zero-knowledge proof of identity.
    \item [B] Implement the RSA algorithm for 1024-bit keys.
    \item [I] Show that if $p$ and $q$ are close, $n = pq$ can be factored easily (Fermat factorization).
\end{enumerate}
