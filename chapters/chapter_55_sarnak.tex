...
The Laplacian on a hyperbolic surface $\Gamma \backslash \H^2$ has both discrete spectrum (cusp forms) and continuous spectrum (Eisenstein series). Understanding this spectrum, especially the existence of cusp forms for general congruence subgroups, is a deep problem with connections to number theory.
...
For a family of L-functions with a given symmetry type $G$ (where $G$ is one of $\U(N)$, $\Sp}(2N)$, or $\O(N)$), the distribution of low-lying zeros (suitably normalized) coincides with the distribution of eigenvalues near $1$ for $G$.
\end{theorem}
...
For congruence subgroups $\Gamma(N) \subset \SL(2,\Z)$, the smallest non-zero eigenvalue of the Laplacian on $\Gamma(N) \backslash \H^2$ satisfies $\lambda_1 \geq 3/16$.
\end{theorem}
...
\begin{enumerate}
    \item Show that the eigenvalues of a $k$-regular graph are bounded between $-k$ and $k$.
    \item Compute the pair correlation of eigenvalues of a GUE random matrix in the large $N$ limit.
    \item Prove that the eigenvalues of a Ramanujan graph satisfy the Alon--Boppana bound.
    \item Show that $\sum_{n \leq x} \mu(n) = o(x)$ is equivalent to the Prime Number Theorem.
    \item Compute the spectrum of the Lubotzky--Phillips--Sarnak graph $X^{5,3}$ explicitly.
    \item Derive the explicit formula relating zeros of the Riemann zeta function to sums over primes.
    \item Show that the Laplacian on a compact hyperbolic surface has discrete spectrum.
\end{enumerate}
