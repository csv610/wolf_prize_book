...
The fundamental group $\pi_1(M)$ of a hyperbolic manifold $M = \H^n / \Gamma$ embeds as a discrete subgroup $\Gamma \subset \Isom(\H^n) \cong \SO(n,1)$. The question of geometric deformations is equivalent to: can $\Gamma$ be continuously deformed inside $\SO(n,1)$ while remaining discrete and of finite covolume?
...
Mostow's proof proceeds by contradiction. Given two discrete subgroups $\Gamma_1, \Gamma_2 \subset \SO(n,1)$ acting cocompactly on $\H^n$ and an isomorphism $\varphi: \Gamma_1 \to \Gamma_2$, one wants to show $\varphi$ extends to an isometry of $\H^n$.
...
\begin{enumerate}
    ...
    \item Conclude that the original groups are conjugate in $\Isom(\H^n)$.
\end{enumerate}
...
The $n$-dimensional \textbf{hyperbolic space} $\H^n$ is the unique simply connected Riemannian manifold of constant sectional curvature $-1$. It can be modeled as:
\[
\H^n = \{ (x_1, \ldots, x_n) \in \R^n : x_n > 0 \}, \quad ds^2 = \frac{dx_1^2 + \cdots + dx_n^2}{x_n^2}
\]
(the upper half-space model).
\end{definition}

\begin{definition}[Hyperbolic Manifold]
A \textbf{hyperbolic manifold} is a Riemannian manifold locally isometric to $\H^n$. Equivalently, it is a quotient $\H^n / \Gamma$ where $\Gamma \subset \Isom(\H^n)$ acts freely and properly discontinuously.
\end{definition}
...
\begin{enumerate}
    \item Show that the upper half-plane $\H^2$ with metric $ds^2 = (dx^2 + dy^2)/y^2$ has constant curvature $-1$.
    \item Prove that $\Isom(\H^n) \cong \SO(n,1)$ (the group of $(n+1)\times(n+1)$ matrices preserving the bilinear form of signature $(n,1)$).
    \item Show that a quasi-isometry between two hyperbolic spaces induces a homeomorphism of their boundaries.
    \item Prove that any conformal map of $S^n$ for $n \geq 2$ extends to a M\"obius transformation.
    \item Compute the dimension of the moduli space of hyperbolic surfaces of genus $g$.
    \item Show that $\SL(2,\R)$ acts on $\H^2$ by fractional linear transformations.
    \item Classify all irreducible bounded symmetric domains using the classification of simple Lie algebras.
\end{enumerate}
...
\begin{description}
    ...
    \item[M\"obius transformation] A conformal automorphism of the sphere $S^n$, extending an isometry of $\H^{n+1}$.
