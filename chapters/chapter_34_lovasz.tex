...
\begin{enumerate}
  \item Let \(A_1, \dots, A_n\) be events each with probability at most \(p\) and each dependent on at most \(d\) others. Show that the symmetric Lov\'{a}sz Local Lemma gives \(\Pr(\bigcap \overline{A_i}) > 0\) whenever \(p \leq 1/(e(d+1))\).
  \item Use the Local Lemma to prove that if a \(k\)-CNF formula has each clause sharing variables with at most \(2^{k-2}/e\) other clauses, then the formula is satisfiable.
  \item Let \(G\) be a graph with maximum degree \(\Delta\). Show that there exists a proper vertex coloring of \(G\) using \(\Delta + 1\) colors. (This is the greedy coloring bound; use the Local Lemma to find a coloring using \(O(\Delta / \log \Delta)\) colors under certain conditions.)
  \item Implement a simple version of the LLL algorithm for a 2-dimensional lattice. Show that the output basis has the property that the angle between the two basis vectors is at least \(60^\circ\).
  \item Prove that the product of the lengths of the vectors in an LLL-reduced basis is at most \(2^{n(n-1)/4} \det(\Lambda)\).
  \item Let \(\Lambda \subset \Z^n\) be a lattice with determinant \(\det(\Lambda)\). Show that the shortest vector \(\lambda_1\) satisfies \(\lambda_1 \leq \sqrt{n} \det(\Lambda)^{1/n}\). (Minkowski's theorem.)
  \item Show that the value of the Lov\'{a}sz theta function for the complete graph \(K_n\) is 1, and for the empty graph \(\overline{K_n}\) it is \(n\).
  \item Compute \(\vartheta(C_5)\) by constructing an explicit orthonormal representation of \(C_5\) in \(\R^5\) using equally spaced unit vectors.
  \item Prove that the Shannon capacity satisfies \(\Theta(G) \geq \alpha(G)\) and \(\Theta(G) \leq \vartheta(G)\) for any graph \(G\).
  \item Let \(G\) be a perfect graph. Show that every induced subgraph of \(G\) satisfies \(\chi(H) = \omega(H)\) and \(\chi(\overline{H}) = \omega(\overline{H})\).
  \item Prove the Lov\'{a}sz replication lemma: If \(G\) is perfect and \(v \in V(G)\), then the graph obtained by replacing \(v\) with two adjacent copies is also perfect.
  \item Show that a bipartite graph is perfect by showing \(\chi(H) = \omega(H)\) for every induced subgraph \(H\).
  \item Let \((G_n)\) be a sequence of random graphs \(G(n, 1/2)\). Show that with probability 1, the sequence is convergent in the sense of graph limits. What is the limit graphon?
  \item Define the cut distance between two graphons and show that it is a metric on the space of graphons up to measure-preserving transformations.
  \item Let \(M\) be a random walk on a connected \(d\)-regular graph \(G\). Compute the conductance \(\Phi\) of the walk. Use the Lov\'{a}sz--Simonovits method to bound the mixing time.
  \item Show that the Glauber dynamics for the Ising model on an expander graph at high temperature has mixing time \(O(n \log n)\) using the Lov\'{a}sz--Simonovits method.
  \item Prove Kneser's conjecture for \(n \geq 2k\): The Kneser graph \(KG(n,k)\) has chromatic number \(n - 2k + 2\). (Use the Borsuk--Ulam theorem.)
  \item Show that the ellipsoid method solves linear programming in polynomial time. Explain why the separation oracle paradigm allows optimization over convex bodies with exponentially many facets.
  \item Let \(\overline{G}\) be the complement of a perfect graph \(G\). Show that \(\vartheta(\overline{G}) = \omega(G) = \chi(G)\).
  \item Consider the Shannon capacity of a graph \(G\). Show that if \(G\) is a perfect graph, then \(\Theta(G) = \alpha(G)\).
  \item Let \(\Lambda\) be the lattice generated by \(v_1 = (1, 0)\) and \(v_2 = (0, 1)\) in \(\R^2\). Apply the LLL algorithm to the basis \(\{(2, 3), (1, 1)\}\). Show that the reduced basis consists of short vectors spanning the same lattice.
  \item Let \(G\) be a graph and let \(\vartheta(G)\) be the Lov\'{a}sz theta function. Prove that \(\vartheta(G)\) is multiplicative under the strong product: \(\vartheta(G \boxtimes H) = \vartheta(G) \vartheta(H)\).
  \item Show that for the 5-cycle \(C_5\), \(\alpha(C_5 \boxtimes C_5) = 5\), establishing the lower bound \(\Theta(C_5) \geq \sqrt{5}\).
\end{enumerate}
