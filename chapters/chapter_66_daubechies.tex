...
\begin{enumerate}
    \item Show that the Haar wavelet ($N=1$) is the simplest Daubechies wavelet.
    \item Compute the filter coefficients $h_k$ for the Daubechies D4 wavelet.
    \item Show that a wavelet with $N$ vanishing moments can represent polynomials of degree $N-1$ exactly.
    \item Implement the lifting scheme for the Haar wavelet.
    \item Show that the JPEG 2000 9/7 wavelet is biorthogonal, not orthogonal.
    \item Compute the smoothness of the D4 wavelet using the Holder exponent.
    \item Show that the FBI wavelet standard achieves 20:1 compression on fingerprint images.
\end{enumerate}
