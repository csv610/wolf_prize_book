...
\begin{enumerate}
    \item Show that a Drinfeld module of rank 1 corresponds to the Carlitz module.
    \item Verify that the quantum group $U_q(\mathfrak{sl}_2)$ satisfies the Hopf algebra axioms.
    \item Compute the universal R-matrix for $U_q(\mathfrak{sl}_2)$ explicitly.
    \item Show that the Drinfeld modular variety parametrizes rank 2 Drinfeld modules.
    \item Derive the KdV hierarchy from the Drinfeld--Sokolov reduction of $\widehat{\mathfrak{sl}}_2$.
    \item Prove that the Langlands correspondence for $\GL(1)$ over function fields is class field theory.
    \item Compute the invariant of the trefoil knot using the quantum group $U_q(\mathfrak{sl}_2)$.
\end{enumerate}
