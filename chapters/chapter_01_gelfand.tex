...
\index{Gelfand, Israel Moiseevich}
...
\begin{itemize}
    \item \textbf{Functional Analysis:} The theory of normed rings (Banach algebras), the Gelfand representation\index{Gelfand transform}, the Gelfand--Mazur theorem, the Gelfand--Naimark theorem, and the Gelfand--Naimark--Segal (GNS) construction\index{GNS construction}.
...
\subsection{C\texorpdfstring{$^*$}{}-Algebras and the GNS Construction}
...
For further exploration, see Chapter~10 (Krein) on functional analysis and operator theory, Chapter~18 (Lax) on functional analysis and PDEs, and Chapter~21 (Calder\'on) on singular integrals and functional analysis.

\section*{Exercises for the Reader}
\addcontentsline{toc}{section}{Exercises}

\begin{enumerate}
    \item [B] Show that $C[0,1]$ with the supremum norm and pointwise multiplication is a commutative Banach algebra. What are its characters?
    \item [I] Prove the spectral radius formula $r(x) = \lim_{n\to\infty} \|x^n\|^{1/n}$ using the Gelfand representation.
    \item [I] For a compact Hausdorff space $X$, show that the Gelfand transform $C(X) \to C(\Phi_{C(X)})$ is an isometric isomorphism.
    \item [B] Verify that $B(H)$ satisfies the C$^*$-identity $\|T^* T\| = \|T\|^2$.
    \item [I] Let $(G,K)$ be a Gelfand pair. Show that $L^2(G/K)$ decomposes with multiplicity one.
    \item [A] For the one-dimensional Schr\"odinger equation $-\psi'' + q\psi = k^2\psi$, derive the Gelfand--Levitan equation from the spectral measure.
    \item [I] Compute the GK dimension of the polynomial ring $k[x_1,x_2,x_3]$ and the Weyl algebra $A_2$.
\end{enumerate}
