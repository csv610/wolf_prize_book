...
Riemann--Roch problem from a theorem about curves into a general
...
\subsection{The Signature Theorem and the \(L\)-Genus}
...
\begin{enumerate}
    \item Let \(C\) be a compact Riemann surface of genus \(g\) and let
    ...
    \item Prove that the Hirzebruch surface \(\Sigma_1\) is isomorphic
    to the blowup of \(\mathbb{P}^2\) at a point. Show that
    \(c_1^2(\Sigma_1) = 8\) and \(c_2(\Sigma_1) = 4\).

    \item For the Hirzebruch surface \(\Sigma_n\), compute the canonical
    divisor \(K_{\Sigma_n}\) and verify the adjunction formula.

    \item Let \(X\) be a compact complex surface. Use HRR to prove the
    Noether formula:
    \[
    \chi(X, \mathcal{O}_X) = \frac1{12}(c_1(X)^2 + c_2(X)).
    \]

    \item Show that the Todd genus of a Calabi--Yau threefold
    (\(c_1 = 0\)) is \(0\). What does HRR give for
    \(\chi(X, \mathcal{O}_X)\)?

    \item Let \(Q(x) = 1 + q_1 x + q_2 x^2 + \cdots\) be a power
