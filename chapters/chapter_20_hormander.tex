...
synthesizing pseudodifferential operators\index{pseudodifferential operator}, Fourier integral operators,
...
and hypoellipticity\index{hypoellipticity}. His four-volume magnum opus, \textit{The Analysis of
...
\begin{enumerate}
    \item [B] Let \(u\) be the characteristic function of the unit ball in
    ...
    \item [B] Prove that for the Dirac delta \(\delta_0\) on \(\R^n\),
    \(\WF(\delta_0) = \{(0,\xi): \xi \neq 0\}\). Show that if a
    distribution has \(\WF(u) = \varnothing\), then \(u \in C^\infty\).
    \item [B] Let \(P = -\partial_x^2 - \partial_y^2\) be the Laplacian on
    ...
    \(\eta =\) constant.
    \item [B] For the wave operator \(\square = \partial_t^2 - \partial_x^2\)
    ...
    \item [I] Verify that Lewy's operator
    \[
    ...
    \item [B] Let \(X_1 = \partial_x\) and \(X_2 = x\partial_y\) on \(\R^2\).
    ...
    \item [I] Prove that if \(a \in S^{m_1}\) and \(b \in S^{m_2}\), then
    the composition symbol \(c(x,\xi) = a(x,D) b(x,\xi)\) (where
    \(b(x,\xi)\) acts on \(a\)) belongs to \(S^{m_1+m_2}\) and has
    leading term \(a(x,\xi) b(x,\xi)\).
    \item [B] Implement the stationary phase method for the one-dimensional
    integral
    \[
    ...
    spatial variables.
    \item [B] Derive the composition formula for two pseudodifferential
    ...
    \item [I] Let \(P\) be an elliptic operator on a compact manifold.
    ...
    \item [A] Show that H\"ormander's condition for local solvability is
    ...
    \item [A] (Project) Read H\"ormander's original 1971 paper on Fourier
