...
\]
where $L_{X_t}$ is the Lie derivative\index{Lie derivative}. By the Cartan formula, $L_{X_t}\omega_t = d(\iota_{X_t}\omega_t)$ (since $d\omega_t = 0$). Thus we need to solve the equation
\[
...
For further exploration, see Chapter~38 (Arnold) on KAM theory and dynamical systems and Chapter~33 (Sinai) on ergodic theory and statistical mechanics.

\section*{Exercises for the Reader}
\addcontentsline{toc}{section}{Exercises}

\begin{enumerate}
\item [I] Let $\omega = (\omega_1, \dots, \omega_n)$ be a vector of frequencies. Show that if the $\omega_i$ and $1$ are linearly independent over $\Q$ (i.e., $\langle k, \omega \rangle = 0$ only for $k = 0$), then the linear flow $\theta \mapsto \theta + \omega t$ on $\mathbb{T}^n$ is dense. Show that the Diophantine condition $|\langle k, \omega \rangle| \geq \gamma/|k|^\tau$ implies that the flow is also uniquely ergodic.

\item [I] Prove that the set of numbers $\omega \in [0,1]$ satisfying the Diophantine condition $|q\omega - p| \geq c/|q|^{\mu}$ for all $p/q \in \Q$ has full Lebesgue measure when $\mu > 1$. (This is a key step in the KAM theorem: most frequencies are Diophantine.)

\item [B] Show that the golden ratio $\phi = (1+\sqrt{5})/2$ satisfies the Diophantine condition with $\mu = 1$: $|q\phi - p| \geq 1/(\sqrt{5}\,q)$ for all $p,q \in \Z$,$q \neq 0$. (This is a classical result in Diophantine approximation related to the continued fraction expansion of $\phi$.)

\item [B] Consider the harmonic oscillator $H(p,q) = \frac{1}{2}(p^2 + q^2)$. Find action-angle variables $(I,\theta)$ for this system and verify that the Hamiltonian depends only on $I$. Compute the frequency $\omega(I) = \partial H/\partial I$ and check the Kolmogorov non-degeneracy condition.

\item [I] For the standard map $\varphi(\theta, r) = (\theta + r',\; r' = r + \varepsilon \sin\theta)$, verify that it preserves the area form $d\theta \wedge dr$ and satisfies the twist condition for small $\varepsilon$. For $\varepsilon = 0$, describe the invariant curves. What happens to these curves as $\varepsilon$ increases?

\item [I] Let $F: B_0 \to B_0$ be a $C^1$ map between Banach spaces with $F(0) = 0$ and $DF(0)$ an isomorphism. Prove the standard implicit function theorem using Newton iteration: set $x_{n+1} = x_n - DF(0)^{-1}F(x_n)$ and show that $x_n \to 0$ quadratically. Why does this iteration fail when the derivative loses derivatives?

\item [I] Let $u \in C^\infty(\mathbb{T}^n)$ and define the smoothing operator $S_t$ by truncating the Fourier expansion at frequency $|k| \leq t$: $S_t u(\theta) = \sum_{|k| \leq t} \hat{u}(k) e^{i\langle k,\theta \rangle}$. Show that $\|S_t u\|_{C^m} \leq C t^{m - k} \|u\|_{C^k}$ for $m \geq k$ and $\|(I - S_t) u\|_{C^k} \leq C t^{-(m-k)} \|u\|_{C^m}$ for $k \leq m$. These are the smoothing estimates used in the Nash--Moser theorem.

\item [I] Let $\varphi$ be an area-preserving diffeomorphism of the unit disk $D^2$ that equals a rotation $R_\theta$ near the boundary. Show that the Calabi invariant $C(\varphi) = \theta \cdot \area(D^2)/2\pi$ by constructing the generating function $S$ such that $dS = \varphi^*(dx \wedge dy) - dx \wedge dy$.

\item [I] In the restricted three-body problem, verify that the Lagrange triangular point $L_4$ is an equilibrium when the three bodies form an equilateral triangle. Linearize the equations of motion around $L_4$ and compute the frequencies. Show that these frequencies are real (implying linear stability) when the mass ratio satisfies $\mu/(1-\mu) < 1/27$.

\item [I] Consider the Toda lattice with $N=2$ particles. Write the equations of motion, find a Lax pair $(L,B)$, and verify that $L$ is a $2 \times 2$ symmetric matrix. Show that the eigenvalues of $L$ are constants of motion and that the system is completely integrable.

\item [B] Let $u \in C^2(B_R)$ be a non-negative solution of the Laplace equation $\Delta u = 0$ in $B_R \subset \R^n$. Prove the classical Harnack inequality
\[
\sup_{B_{R/2}} u \leq C \inf_{B_{R/2}} u
\]
...
\begin{enumerate}
    \item Moser, J. (1962). On invariant curves of area-preserving mappings of an annulus. \textit{Nachrichten der Akademie der Wissenschaften in Göttingen, Mathematisch-Physikalische Klasse}, 1962(1), 1-20.
    \item Moser, J. (1968). \textit{Lectures on Hamiltonian Systems}. American Mathematical Society.
    \item Moser, J. (1973). \textit{Stable and Random Motions in Dynamical Systems: With Special Emphasis on Celestial Mechanics}. Princeton University Press.
    \item Moser, J. (1983). \textit{Selected Chapters in the Theory of Hamiltonian Systems}. American Mathematical Society.
\end{enumerate}

\subsection*{Biographies and Overviews}
\begin{enumerate}
    \item Zehnder, E. (2000). Jürgen Moser (1928-1999). \textit{Notices of the American Mathematical Society}, 47(1), 108-114.
    \item Arnold, V. I. (2000). Jürgen Moser (1928-1999). \textit{Russian Mathematical Surveys}, 55(1), 1-10.
    \item Pöschel, J. (2000). Jürgen Moser (1928-1999). \textit{SIAM Review}, 42(1), 147-151.
    \item MacTutor History of Mathematics Archive. (n.d.). \textit{Jürgen Kurt Moser}. University of St Andrews. Available at: \url{https://mathshistory.st-andrews.ac.uk/Biographies/Moser_Jurgen/}.
\end{enumerate}

\subsection*{Textbooks and Expository Works}
\begin{enumerate}
    \item Arnold, V. I. (1989). \textit{Mathematical Methods of Classical Mechanics} (2nd ed.). Springer-Verlag.
    \item Siegel, C. L., \& Moser, J. K. (1971). \textit{Lectures on Celestial Mechanics}. Springer-Verlag.
    \item Zehnder, E. (1988). \textit{Lectures on Dynamical Systems: Hamiltonian Systems and Symplectic Geometry}. Birkhäuser.
    \item Pöschel, J., \& Zehnder, E. (1995). \textit{KAM Theory Revisited}. Birkhäuser.
\end{enumerate}
