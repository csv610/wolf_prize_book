...
\begin{enumerate}
    \item Show that the Zariski topology on $\A^1_k$ is the cofinite topology (closed sets are finite sets and the whole space).
    \item Prove that $\A^1_k$ with the Zariski topology is not Hausdorff.
    \item Show that the blow-up of $\A^2$ at the origin is covered by two affine charts, each isomorphic to $\A^2$.
    \item Let $C: y^2 = x^3$ in $\A^2_k$. Normalize $C$ by taking the integral closure of $k[x,y]/(y^2 - x^3)$.
    \item Show that the normalization of a curve is smooth in dimension one.
    \item Let $f: \Spec B \to \Spec A$ be a finite morphism of affine curves. Prove that the fibers are finite sets.
    \item Show that the Zariski topology on $\mathbb{P}^n_k$ is obtained by gluing the Zariski topologies on the standard affine charts $U_i \cong \A^n_k$.
    \item Let $A \subset B$ be an integral extension of domains. Show that $\dim A = \dim B$ (Krull dimension).
    \item Prove that $\Spec R$ is quasi-compact (every open cover has a finite subcover).
    \item Let $X = \{x^2 + y^2 = z^2\} \subset \A^3$ be the cone. Show that the blow-up of $\A^3$ at the origin restricts to a resolution of $X$ with exceptional divisor $\mathbb{P}^1 \times \mathbb{P}^1$.
    \item Show that the complement of a nodal plane cubic curve $\{y^2 = x(x-1)(x-\lambda)\} \subset \mathbb{P}^2$ has abelian fundamental group.
    \item Prove that a dominant morphism of irreducible varieties induces an injection of function fields (the reverse direction of birational equivalence).
\end{enumerate}
