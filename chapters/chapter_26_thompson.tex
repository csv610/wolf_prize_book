...
\section{Thompson's Theorem on Normal \(p\)-Complements}
...
\subsection{The Concept of \(p\)-Constraint}
...
\subsection{The Thompson Group \(Th\)}
...
\begin{enumerate}
\item Prove that a finite group of odd order cannot be simple unless it is cyclic of prime order. (This is an easy consequence of the Feit--Thompson theorem, but try to prove it directly using the class equation for a group of order \(p^a m\) with \(p\) the smallest prime divisor.)
\item Let \(P\) be a \(p\)-group. Show that the Thompson subgroup \(J(P)\) is characteristic in \(P\).
\item Let \(G\) be a finite group with a normal \(p\)-complement \(N\). Show that the Sylow \(p\)-subgroups of \(G\) are all conjugate in \(G\) and that \(G/N \cong P\) for any Sylow \(p\)-subgroup \(P\) of \(G\).
\item Let \(G\) be a finite group and let \(p\) be an odd prime. If \(C_G(x)\) has a normal \(p\)-complement for every element \(x\) of order \(p\) in \(G\), prove that \(G\) has a normal \(p\)-complement. (This is Thompson's theorem; you may use the transfer homomorphism.)
\item Let \(Th\) denote the Thompson group. Assuming that the Schur multiplier of \(Th\) is trivial, show that any central extension of \(Th\) splits.
\item Let \(G\) be an N-group. Prove that every simple section of \(G\) is either a known simple group or an N-group itself.
\item Let \(P\) be a Sylow 2-subgroup of the Thompson group \(Th\). Compute the order of \(P\) from the order formula for \(Th\).
\item Show that the alternating group \(A_5\) is an N-group but not a solvable group. (This illustrates the nontriviality of Thompson's classification.)
\item Let \(G\) be a minimal simple group (a non-abelian simple group all of whose proper subgroups are solvable). Show that the Sylow 2-subgroup of \(G\) has order at most 4. (This is a key step in the Feit--Thompson proof.)
\item Prove that the odd order theorem implies that every finite division ring of odd order is a field.
\item Let \(G\) be a finite group with a strongly embedded subgroup \(M\). Show that \(M\) contains all involutions of \(G\). (This is a key property used in Bender's theorem.)
\item Let \(P\) be a finite \(p\)-group. Prove that \(J(P)\) is nontrivial whenever \(P\) is nontrivial.
\item Let \(G\) be a minimal counterexample to the odd order theorem. Show that \(G\) has no proper subgroup of index less than some function of \(|G|\). (This is a necessary step in the Feit--Thompson character theory.)
\item Let \(P\) be a Sylow \(p\)-subgroup of a finite group \(G\) and let \(J(P)\) be the Thompson subgroup. Show that if \(N_G(J(P))\) controls the \(p\)-fusion in \(G\), then the transfer map \(V_G^P\) is trivial.
\item Prove that the Monster group is rigid by analyzing its character table and showing the existence of a suitable rigid triple of conjugacy classes. (This is a computational exercise using the known character table of the Monster.)
\item Let \(G\) be a finite group and let \(p\) be a prime. Prove that if \(G\) is \(p\)-constrained and \(p\)-stable, then the normalizer of the Thompson subgroup of a Sylow \(p\)-subgroup controls the \(p\)-fusion in \(G\).
\item Show that the alternating group \(A_7\) is an N-group. Classify the local subgroups of \(A_7\) and verify that each is solvable.
\item Let \(V\) be a vector space over a field of characteristic \(p\) and let \(x \in \GL(V)\) be an element of order \(p\). Show that \(x\) acts quadratically on \(V\) (that is, \((x-1)^2 = 0\)) if and only if the Jordan blocks of \(x\) have size at most 2. This is the simplest example of a quadratic pair.
\item Let \(Th\) be the Thompson group and let \(g \in Th\) be an element of order 3. Compute the order of the centralizer \(C_{Th}(g)\) using the character table of \(Th\).
\item Show that the Thompson order formula reduces to the class equation when \(\pi\) consists of a single prime.
\item Let \(G\) be a finite group with a strongly embedded subgroup \(M\). Prove that the number of involutions in \(G\) is equal to the number of involutions in \(M\).
\end{enumerate}
