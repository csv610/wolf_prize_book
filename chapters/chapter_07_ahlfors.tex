...
Ahlfors's work is characterized by a profound geometric intuition. He transformed the theory of Riemann surfaces, created the modern theory of quasiconformal mappings\index{quasiconformal mapping}, and made fundamental contributions to Kleinian groups, Nevanlinna theory, and conformal geometry. His book ``Complex Analysis'' (1953) is one of the most widely read mathematics textbooks ever written.
...
\begin{itemize}
    \item \textbf{Riemann Surfaces:} The Ahlfors finiteness theorem (every finitely generated Kleinian group has a finite-sided fundamental polyhedron); the theory of covering surfaces; the Ahlfors--Bers proof of Teichm{\"u}ller's theorem\index{Teichm\"uller space}.
...
For further exploration, see Chapter~13 (Kodaira) on complex manifolds and K\"ahler geometry and Chapter~50 (Yau) on complex geometry and Calabi--Yau manifolds.

\section*{Exercises for the Reader}
\addcontentsline{toc}{section}{Exercises}

\begin{enumerate}
    \item [I] Show that a $K$-quasiconformal mapping satisfies the Beltrami equation $f_{\bar z} = \mu f_z$ with $\|\mu\|_\infty \leq (K-1)/(K+1)$.
    \item [B] Prove that the composition of a $K_1$-quasiconformal map with a $K_2$-quasiconformal map is $K_1 K_2$-quasiconformal.
    \item [B] Show that the unit disk $\mathbb{D}$ is conformally equivalent to the upper half-plane $\mathbb{H}$ by the Cayley transform $z \mapsto i(1-z)/(1+z)$.
    \item [B] Compute the Poincar{\'e} metric on $\mathbb{H}$ from the metric on $\mathbb{D}$ via the Cayley transform.
    \item [B] Prove the classical Schwarz lemma: if $f: \mathbb{D} \to \mathbb{D}$ is holomorphic with $f(0) = 0$, then $|f(z)| \leq |z|$ and $|f'(0)| \leq 1$.
    \item [B] Verify that $f(z) = (z - a)/(1 - \bar a z)$ is an automorphism of $\mathbb{D}$ for any $a \in \mathbb{D}$.
    \item [B] Show that the M\"{o}bius transformation $z \mapsto (az+b)/(cz+d)$ sends circles and lines to circles and lines.
    \item [I] Let $f(z) = e^z$. Find the asymptotic values of $f$ and verify the Denjoy--Carleman--Ahlfors theorem for this function.
    \item [B] Show that the fundamental group of a compact Riemann surface of genus $g \geq 2$ is $\pi_1(S_g) = \langle a_1,b_1,\dots,a_g,b_g \mid \prod [a_i,b_i] = 1 \rangle$.
    \item [B] Write the Beltrami equation for $f(z) = z + \epsilon \bar z$ with $|\epsilon| < 1$. Show that this is a quasiconformal map and compute its dilatation.
    \item [I] Prove that the area of a hyperbolic $n$-gon with interior angles $\alpha_1,\dots,\alpha_n$ is $(n-2)\pi - \sum \alpha_i$.
    \item [I] Show that if $f: \mathbb{C} \to \mathbb{C}$ is a non-constant entire function, then the equation $f(z) = a$ has a solution for every $a \in \mathbb{C}$ except at most one value (Picard's little theorem).
\end{enumerate}
