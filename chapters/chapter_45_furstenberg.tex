...
\begin{theorem}[Furstenberg Recurrence Theorem\index{Furstenberg recurrence theorem}, 1977]
...
\begin{theorem}[Szemer\'edi's Theorem\index{Szemer\'edi theorem}, 1975]
...
Furstenberg's fundamental insight was that the long-term behavior of the random walk is captured by a measure-theoretic boundary, now called the \textbf{Furstenberg boundary}\index{Furstenberg boundary} or the \textbf{Poisson boundary} of the random walk. The boundary is a \(G\)-space \(\partial G\) equipped with a family of harmonic measures \(\nu_x\) (one for each starting point \(x\)) such that the random walk converges to a random point on the boundary, and the distribution of this limit point encodes the asymptotic behavior of the walk.
...
\begin{description}
    \item[Ergodic Theory] A branch of mathematics that studies dynamical systems with an invariant measure. Furstenberg made fundamental contributions, particularly in applying ergodic methods to problems in number theory and combinatorics.
    \item[Furstenberg's Recurrence Theorem] A key result in ergodic theory stating that for any measure-preserving system $(X, \mathcal{B}, \mu, T)$ and any set $A \in \mathcal{B}$ with $\mu(A) > 0$, there exists an integer $n \ge 1$ such that $\mu(A \cap T^{-n}A \cap T^{-2n}A \cap \dots \cap T^{-kn}A) > 0$ for any $k \ge 1$. This theorem is a powerful tool for proving combinatorial results.
    \item[Szemeredi's Theorem] A theorem in combinatorial number theory stating that any subset of the integers with positive upper density contains arbitrarily long arithmetic progressions. Furstenberg provided a groundbreaking proof of this theorem using methods from ergodic theory.
    \item[IP Set] An "infinite polynomial" set. A set $S \subseteq \mathbb{N}$ is an IP set if it contains all finite sums of distinct elements from an infinite sequence $(x_n)_{n=1}^\infty$ of positive integers. These sets play a role in Furstenberg's work on recurrence and combinatorial number theory.
    \item[Profinite Topology] A topology on the integers (or other groups) where open sets are unions of arithmetic progressions. Furstenberg used this topological perspective to study recurrence and prove results like Szemeredi's Theorem, connecting dynamics with number theory.
    \item[Unique Ergodicity] A property of a dynamical system where there is only one invariant probability measure. Furstenberg's work often involves constructing or analyzing systems with unique ergodicity, particularly in the context of compact group extensions and random walks.
    \item[Poisson Boundary] For a random walk on a group, the Poisson boundary is a space that captures the asymptotic behavior of the random walk. Furstenberg made significant contributions to the theory of Poisson boundaries, especially in the context of products of random matrices and their applications to Lie groups.
\end{description}
...
\begin{enumerate}
    \item Furstenberg, H. (1967). Disjointness in ergodic theory, minimal sets, and a problem in Diophantine approximation. \textit{Mathematical Systems Theory}, 1(1), 1-49.
    \item Furstenberg, H. (1977). Ergodic behavior of diagonal measures and a theorem of Szemeredi on arithmetic progressions. \textit{Journal d'Analyse Mathématique}, 31(1), 204-256.
    \item Furstenberg, H. (1981). \textit{Recurrence in Ergodic Theory and Combinatorial Number Theory}. Princeton University Press.
\end{enumerate}

\subsection*{Biographies and Overviews}
\begin{enumerate}
    \item Assaf, S., \& Glasner, E. (Eds.). (2019). \textit{Ergodic Theory and Dynamical Systems: A Volume in Honor of Hillel Furstenberg}. American Mathematical Society.
    \item Glasner, E. (2020). The work of Hillel Furstenberg. \textit{Notices of the American Mathematical Society}, 67(10), 1545-1554.
    \item The Norwegian Academy of Science and Letters. (2020). \textit{The Abel Prize Laureates 2020: Hillel Furstenberg and Gregory Margulis}. Retrieved from \url{https://www.abelprize.no/c76307/seksjon/vis.html?tid=76308} (Accessed October 26, 2023).
\end{enumerate}

\subsection*{Textbooks and Expository Works}
\begin{enumerate}
    \item Einsiedler, M., \& Ward, T. (2011). \textit{Ergodic Theory with a View Towards Number Theory}. Springer.
    \item Glasner, E. (2003). \textit{Ergodic Theory Old and New}. Birkhäuser.
    \item Walters, P. (1982). \textit{An Introduction to Ergodic Theory}. Springer.
\end{enumerate}
