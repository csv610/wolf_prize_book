...
A \textbf{Tannakian category} over a field \(k\) is a rigid abelian tensor category \(\mathcal{C}\) equipped with a fiber functor \(\omega: \mathcal{C} \to \Vec_k\) to the category of finite-dimensional vector spaces over \(k\), satisfying certain exactness and faithfulness conditions.
...
\begin{enumerate}
    ...
    \item \(\omega: \mathcal{C} \to \Vec_k\) is an exact, faithful \(k\)-linear tensor functor (the \textbf{fiber functor}).
...
Let \(\mathcal{C}\) be a symmetric tensor category over an algebraically closed field \(k\) of characteristic zero. Suppose that for every object \(X \in \mathcal{C}\), the length of \(X^{\otimes n}\) is bounded by a polynomial in \(n\). Then \(\mathcal{C}\) is Tannakian: it admits a fiber functor to \(\Vec_k\).
\end{theorem}
...
\begin{enumerate}
    \item Let \(X\) be a smooth projective curve of genus \(g\) over \(\F_q\). Show that the zeta function of \(X\) is
    \[
    ...
    \item Prove that for a smooth projective variety \(X\) over \(\F_q\), the functional equation for the zeta function is equivalent to Poincar\'e duality for \'etale cohomology.
    \item Let \(X\) be a smooth projective surface over \(\F_q\). Show that the zeta function takes the form
    \[
    ...
    \item Show that the category \(\Rep_k(G)\) of finite-dimensional representations of an affine group scheme \(G\) over a field \(k\) is a neutral Tannakian category with fiber functor given by the forgetful functor.
    \item Prove that the Deligne--Mumford compactification \(\overline{\M}_g\) is proper by showing that any family of smooth curves over a punctured disk can be extended to a family of stable curves after a finite base change.
    \item Let \(V\) be a pure Hodge structure of weight \(k\). Show that the Hodge decomposition \(V_\C = \bigoplus_{p+q=k} V^{p,q}\) determines and is determined by the Hodge filtration \(F^p V_\C = \bigoplus_{r \geq p} V^{r,k-r}\).
    \item For a smooth variety \(U = X \setminus D\) where \(D\) is a divisor with simple normal crossings, show that the cohomology of \(U\) is computed by the complex \(\Omega_X^\bullet(\log D)\).
    \item Verify that for the modular discriminant \(\Delta(q) = q \prod (1-q^n)^{24}\), the Ramanujan conjecture for weight 12 gives \(|\tau(p)| \leq 2 p^{11/2}\).
    \item Let \(f\) be a cusp form of weight \(k \geq 2\). Show that the associated Galois representation \(\rho_{f,l}\) is unramified outside \(lN\) where \(N\) is the level, and that for \(p \nmid lN\), \(\operatorname{Tr} \rho_{f,l}(\Frob_p) = a_p\).
    \item Prove that a Deligne--Mumford stack with trivial automorphism groups is an algebraic space.
    \item Let \(\mathcal{C}\) be a Tannakian category with fiber functor \(\omega\). Show that the group scheme \(\underline{\Aut}^\otimes(\omega)\) is affine.
    \item For a smooth projective variety \(X\) over \(\C\), show that the Hodge numbers satisfy \(h^{p,q} = h^{q,p}\) and \(h^{p,q} = h^{n-p,n-q}\) (Hodge symmetry and Serre duality).
    \item Let \(X\) be a smooth quasi-projective variety. Use Deligne's mixed Hodge theory to show that the weight filtration on \(H^k(X, \Q)\) satisfies \(W_{k-1} = 0\) and \(W_k = H^k\).
    \item Show that the zeta function of a smooth projective curve \(X\) of genus \(g\) over \(\F_q\) satisfies the Riemann hypothesis if and only if the eigenvalues of Frobenius on \(H^1_{\text{\'et}}(X_{\overline{\F}_q}, \Q_l)\) have absolute value \(q^{1/2}\).
    \item Let \(\mathcal{C}\) be a rigid tensor category over a field \(k\). Show that the endomorphism algebra of the unit object \(\mathbf{1}\) is a field extension of \(k\).
\end{enumerate}
