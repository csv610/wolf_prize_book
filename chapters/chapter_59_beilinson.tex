...
\begin{enumerate}
    \item Show that the category of $\mathfrak{sl}_2$-modules is equivalent to D-modules on $\P^1$.
    \item Compute the cohomology of a perverse sheaf on a curve.
    \item State the decomposition theorem for a resolution of singularities of a surface.
    \item Show that the flag variety $G/B$ is a projective algebraic variety.
    \item Define the regulator map from $K_1$ of a number field to its Minkowski embedding.
    \item Give an example of a Hecke eigensheaf for $\GL(1)$.
    \item Compute the K-theory of $\P^1$ and relate it to the Beilinson regulator.
\end{enumerate}
