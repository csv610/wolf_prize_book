...
\begin{enumerate}
    \item [B] Show that $S^1$ embeds in $\mathbb{R}^2$ but not in $\mathbb{R}^1$.
    \item [B] Show that $\mathbb{RP}^1$ is diffeomorphic to $S^1$.
    \item [I] Show that $S^n$ has a non-vanishing vector field if and only if $n$ is odd. (Hint: use the Euler characteristic and the Poincar{\'e}--Hopf theorem.)
    \item [I] Compute the Stiefel--Whitney classes of the tangent bundle $T\mathbb{RP}^n$ using the Whitney sum formula.
    \item [I] Show that $w_1(E) = 0$ if and only if $E$ is orientable.
    \item [I] Show that the Klein bottle can be immersed in $\mathbb{R}^3$ but not embedded.
    \item [A] Prove that any smooth map $f: M \to \mathbb{R}^N$ with $N > 2m$ can be perturbed to an embedding.
    \item [B] Show that the Whitney umbrella $x^2 = y^2 z$ is not a smooth submanifold of $\mathbb{R}^3$.
    \item [I] Compute the cup product structure on $H^*(\mathbb{RP}^2; \mathbb{Z}_2)$.
    \item [I] Find a Whitney stratification of the nodal curve $y^2 = x^3 + x^2$.
    \item [B] Show that the tangent bundle of $S^2$ is not trivial (hairy ball theorem).
    \item [I] Prove that the Whitney sum $E \oplus F$ of two vector bundles has Stiefel--Whitney classes given by $w_k(E \oplus F) = \sum_{i+j=k} w_i(E) \cup w_j(F)$.
    \item [B] Show that a matroid can be equivalently defined in terms of its bases, its circuits, or its rank function.
    \item [A] Construct a Whitney stratification of the cone $x^2 + y^2 = z^2$ in $\mathbb{R}^3$ and verify the Whitney conditions.
\end{enumerate}
