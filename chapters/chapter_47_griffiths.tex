...
Given a family $\pi: \mathcal{X} \to B$ of algebraic varieties, one can consider the \textit{period map} $\Phi: B \to D$ sending each point $b \in B$ to the Hodge structure on $H^k(X_b, \C)$. The image lies in a classifying space $D$ of Hodge structures (a Griffiths period domain).
...
\]
where $F^k$ is the Hodge filtration. These are complex tori that parametrize certain cohomology classes.
...
\begin{enumerate}
    \item Show that the Hodge numbers $h^{p,q}$ of a compact K\"ahler manifold satisfy $h^{p,q} = h^{q,p}$ and $h^{p,q} = h^{n-p,n-q}$ where $n = \dim_\C X$.
    \item Verify that the Griffiths transversality condition $\nabla \mathcal{F}^p \subset \mathcal{F}^{p-1} \otimes \Omega_B^1$ is equivalent to the vanishing of the $(0,2)$ part of the curvature of the Gauss--Manin connection.
    \item For an elliptic curve $E = \C / (\Z \oplus \tau \Z)$, compute the period matrix and show that the period map from the upper half-plane to the Siegel upper half-space is holomorphic and satisfies transversality.
    \item Let $X$ be a smooth projective curve of genus $g$. Show that $J^1(X)$ is isomorphic to the Jacobian $\Pic^0(X)$ and that the Abel--Jacobi map is an isomorphism.
    \item Prove that the intermediate Jacobian $J^k(X)$ is a complex torus. Compute its dimension in terms of Hodge numbers.
    \item Show that if $X$ is a Calabi--Yau threefold, then the Griffiths transversality condition for the variation of $H^3(X)$ forces the period map to satisfy the Picard--Fuchs equation.
    \item Let $D$ be a period domain for weight $k$ Hodge structures. Show that the horizontal tangent bundle $T^h D$ is a subbundle of $T D$ of codimension $\sum_{p} \dim \Hom(H^{p+1,q-1}, H^{p,q})$.
\end{enumerate}
